...
- **MCP 传输修复**：从 SSE 切换为 Streamable HTTP，修复远程部署下 session 丢失问题
- **端点更新**：Hermes 通过 `/mcp` 端点接入
- **Compose 更新**：默认使用 `kid941005/smart3w-mcp:latest`
...
- **Hermes 可直接调用**：通过 Streamable HTTP `/mcp` 端点暴露，配置 `mcp_servers.smart3w` 即可接入
